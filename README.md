...
新增 ESP32-CAM 影像節點,幀差演算法偵測持續活動 → 在哭聲發生**之前**提前預警,並在每次通知附上現場截圖,實現「預警 → 警報」分級通知。判斷用「最近 3 分鐘活動比例」滑動視窗,避免把睡眠中的短暫翻身誤判為醒來。

## 🎚️ 設計亮點:資料驅動的門檻校準

哭聲判定的「分數門檻」不是拍腦袋定的,專案內建校準工具鏈:

1. 系統先以 `LOG_ONLY = True` 模式運行 3~5 天,只記錄不通知
2. 哭聲分數寫入 `cry_scores.csv`、活動量寫入 `motion_scores.csv`,收集真實環境數據
3. 執行 `analyze_calibration.py`:自動統計背景噪音與疑似哭聲的分數分佈,**輸出建議門檻值**與校準報告
4. 將建議的 `CRY_SCORE_THRESHOLD` 填回設定,搭配持續秒數,切回通知模式
...
- [x] 聲音哭聲偵測(YAMNet)
- [x] 影像活動預警(幀差 + 3 分鐘滑動視窗)
- [x] Telegram 通知與遠端指令
- [x] 自動校準工具(`analyze_calibration.py`)

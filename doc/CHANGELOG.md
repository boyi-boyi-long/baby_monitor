...
## 2026-07-07　硬體實測：INMP441 接線＋燒錄＋端到端跑通，新增 /sound 指令

**新增**
- `/sound` 指令（`config.SOUND_COMMAND`）：Telegram 傳 `/sound` 給 Bot，回傳最近一段
  現場錄音（WAV）＋目前哭聲分數狀態，方便不用等真的哭聲觸發警報也能測麥克風/校準。
- `backend/audio_monitor.py`：把音訊緩衝區與狀態從 `main()` 區域變數重構成
  `AudioMonitor` class（跟既有 `video_monitor.ActivityMonitor` 同樣模式），
  新增 `get_status_text()`、`get_recent_clip()` 給 `/sound` 指令讀取。
- `config.py`/`config.example.py`：新增 `SOUND_COMMAND`。

**遇到的問題與解決方法**

1. **Arduino IDE 燒錄失敗「未提供上傳連接埠」**
   原因：電腦沒抓到 ESP32 的 COM 埠。檢查裝置管理員「連接埠 (COM 和 LPT)」，
   確認 CP2102 驅動有正確安裝、USB 線是可傳輸資料的線（不是純充電線），
   手動在 Arduino IDE 選對 COM 埠後燒錄成功。

2. **韌體燒錄成功、Serial Monitor 也顯示「開始串流音訊」，但筆電端完全收不到 UDP 封包**
   （`/sound` 回報「尚未收到麥克風資料」）。
   排查發現：韌體裡 `LAPTOP_IP` 填的是 `172.17.244.116`——這其實是筆電上
   **VMware 虛擬網卡（VMnet）的 IP**，不是真實 Wi-Fi 網卡的 IP。
   這台筆電裝了 VMware，`ipconfig` 會列出多張虛擬介面卡，容易抄錯行。
   用 `ipconfig` 找到「無線區域網路介面卡 Wi-Fi」區塊底下的 IPv4（`10.244.134.160`，
   跟 ESP32 自己的 IP `10.244.134.230`、ESP32-CAM 的 `10.244.134.75` 同網段），
   改回韌體重新燒錄後解決。
   **教訓**：往後任何「筆電區網 IP」的設定，都要對照 ESP32 開機時印出的自身 IP
   是否同網段，不能只看 `ipconfig` 第一個跳出來的介面卡。

3. **端到端驗證結果**：對著 INMP441 播放哭聲，終端機分數穩定落在 `0.6~0.99`
   （`最強類別: Crying, sobbing`），環境音/安靜時分數落在 `0.00`，
   YAMNet 對哭聲與非哭聲的區隔清楚，滑動視窗有觸發警報邏輯。
   **哭聲警報＋錄音片段＋`/sound` 指令的完整路徑已跑通。**

**下一步**
- 確認 Telegram 有沒有實際收到 🚨 哭聲警報訊息（附截圖＋10秒錄音），
  目前只確認了終端機分數與觸發邏輯，還沒逐一核對 Telegram 端收到的內容是否正確。
- ESP32-CAM 目前串流連線仍會失敗（見終端機 `Connection to tcp://...:81 failed`），
  影像端尚未實測，`/photo`／`/status` 指令與活動量預警待接上鏡頭後驗證。
- 更新 `CLAUDE.md` 的「目前進度」：階段 1+2 的「待接線燒錄實測」可以拿掉了。
- 進入階段 4 校準前，先確認 ESP32-CAM 也接上、`/photo`／`/status` 都測過，
  才算兩條偵測線都完整跑過一輪。
